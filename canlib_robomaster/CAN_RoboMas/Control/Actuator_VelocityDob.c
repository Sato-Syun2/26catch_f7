...
static bool parameters_are_valid(
    const RoboMas_Actuator_VelocityDob_Parameters *const parameters,
    const float dt)
{
...
void RoboMas_Actuator_VelocityDob_Reset(RoboMas_Actuator_VelocityDob_State *const state)
{
    if (state == NULL) {
        return;
    }

    *state = (RoboMas_Actuator_VelocityDob_State){0};
}

float RoboMas_Actuator_VelocityDob_Update(
    const RoboMas_Actuator_VelocityDob_Parameters *const parameters,
    RoboMas_Actuator_VelocityDob_State *const state,
    ...
    const float dt)
{
    if (state == NULL || !parameters_are_valid(parameters, dt) ||
        ...
        RoboMas_Actuator_VelocityDob_Reset(state);
        return 0.0f;
    }
