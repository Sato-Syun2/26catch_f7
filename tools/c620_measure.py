...
class C620Test(Node):
    def __init__(self, directory, action, survey_current=7.0):
    ...
    def check(self):
        ...
        current_guard = 5.8 if self.action == 'calibrate' else 7.8
        if self.action == 'survey':
            current_guard = self.survey_current + 0.8
        if self.action == 'mpc':
            current_guard = getattr(self, 'current_guard', current_guard)
        velocity_guard = getattr(self, 'velocity_guard',
                                 30 if self.action in ('dob', 'chirp', 'move', 'mpc') else 150)
        if abs(self.fb.current) > current_guard or abs(self.fb.velocity) > velocity_guard:
...
def main():
    ...
    parser.add_argument('--survey-current', type=float, default=7.0)
    parser.add_argument('--calibration-speed', type=float, default=10.0)
    args = parser.parse_args()
    if not math.isfinite(args.calibration_speed) or not 0 < args.calibration_speed <= 10:
        parser.error('calibration speed must be within (0, 10]mm/s')
    if not math.isfinite(args.survey_current) or not 0 < args.survey_current <= 7:
        parser.error('survey current must be within (0, 7]A')
    ...
    try:
        ...
        if args.action == 'inspect':
            ...
            if args.action == 'configure':
                if args.command not in ('c620_range','c620_restore','c620_current','c620_speed','c620_kp','c620_ki','c620_dob','c620_alpha'):
                    raise RuntimeError('not a tuning configuration command')
                node.service(args.command, args.target)
            elif args.action == 'calibrate':
                node.service('c620_calibrate', args.calibration_speed)
                node.loop(61, stop_when_disabled=True)
                if node.status()[0] != 2:
                    raise RuntimeError('calibration did not complete')
            elif args.action == 'survey':
                node.service('c620_survey', args.survey_current)
                node.loop(61, lambda t: node.send(10.0,1), stop_when_disabled=True)
                if node.status()[0] != 4:
                ...
                if mode == 4 and abs(args.target) > 20:
                    raise RuntimeError('DOB velocity maximum 20')
                if mode != 4 and not (0 if mode == 5 else 5) <= args.target <= maximum:
